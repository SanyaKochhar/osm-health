...
func GetMatchingServices(kubeClient kubernetes.Interface, podLabels map[string]string, namespace string) ([]service.MeshService, error) {
	serviceList, err := kubeClient.CoreV1().Services(namespace).List(context.Background(), v12.ListOptions{})
	if err != nil {
		return nil, err
	}
	matchingServiceSet := mapset.NewSet()
	for _, svc := range serviceList.Items {
		...
		if selector.Matches(labels.Set(podLabels)) {
			...
			matchingServiceSet.Add(meshSvc)
		}
	}
	var matchingServiceList []service.MeshService
	for matchedSvc := range matchingServiceSet.Iter() {
		matchingServiceList = append(matchingServiceList, matchedSvc.(service.MeshService))
	}
